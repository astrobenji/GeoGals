...
def read_emcee_result(gal_ID, diag):
	...
	'''
	return emcee.backends.HDFBackend('../Results/emcee_backends/subsamp_split/{0}_{1}_outer.hdf5'.format(gal_ID, diag), read_only=True)
...
plot_path	 = '../Plots/emcee_visual_checks/subsamp_split/outer_tests_'
RESOLUTION	 = 100 # dpi
...
tighter_phi_prior_outer_Scal_gals = ['IC5332', 'NGC2835', 'NGC5068']


diags = ['N2S2Ha', 'O3N2', 'Scal']#'N2S2Ha', 'O3N2', 'Scal']# 'N2S2Ha', 'O3N2', 'Scal']

for gal_ID in tighter_phi_prior_outer_Scal_gals:
	for diag in ['Scal']:
...
#		plt.close(fig)
		...
		# flat_blobs   = result.get_blobs(flat=True, discard=burn_in)
		# flat_Zc	 = flat_blobs['Z_c']
		# flat_gradZ = flat_blobs['grad_Z']
		# 
		# flat_Zc_grad_Z = np.stack((flat_Zc, flat_gradZ), axis=1)
		# flat_samples_w_4_params = np.concatenate((flat_samples, flat_Zc_grad_Z), axis=1)
		...
		df.to_pickle('../Results/emcee_exp_fits/all_trained_params/{0}_{1}_outer.pkl'.format(gal_ID, diag))
		...
		Hii_df = gg.open_outer_Hii_df(gal_ID)
		...
		df.to_pickle('../Results/emcee_exp_fits/derived_params/{0}_{1}_outer.pkl'.format(gal_ID, diag))
