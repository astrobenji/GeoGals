...
plot_path = '../Plots/Results/smallscale_vs_large_scale/log_phi_vs/'
...
for diag in diags:
	...
	ax.errorbar(x_ax, phi_centres, yerr=phi_errs, fmt='o', color='red', capsize=10)
	ax.set_ylabel('$\phi$')
	ax.set_xlabel(x_label)
	ax.set_yscale('log')
	plt.tight_layout()
	fig.savefig(plot_path + 'phi_vs_M*_{0}.png'.format(diag), dpi=200)
	
	# All
	x_axes   = [log_M, log_SFR, D_SFMS, log_sSFR]
	x_labels = ['log_M*', 'log(SFR)', 'D_SFMS', 'log_sSFR']
	for x_ax, x_label in zip(x_axes, x_labels):
		...
		ax.set_yscale('log')
		plt.tight_layout()
		corr = np.corrcoef(x_ax, np.log10(phi_centres))
		print(f'corr(log_phi, {x_label}) = {corr[0][1]}')
