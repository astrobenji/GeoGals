...
def read_emcee_result(gal_ID, diag):
	...
	'''
	return emcee.backends.HDFBackend('../Results/emcee_backends/subsamp/{0}_{1}.hdf5'.format(gal_ID, diag), read_only=True)


plot_path	 = '../Plots/emcee_visual_checks/subsamp/'
RESOLUTION	 = 100 # dpi
...
diags = ['N2S2Ha', 'O3N2', 'Scal']# 'N2S2Ha', 'O3N2', 'Scal']

Scal_gals = ['NGC7496', 'NGC4303', 'NGC1672', 'NGC1566', 'NGC1512', 'NGC1087']

for gal_ID in Scal_gals:
	for diag in ['Scal']:
...
#		plt.close(fig)
		...
		df.to_pickle('../Results/emcee_exp_fits/all_trained_params/subsamp/{0}_{1}.pkl'.format(gal_ID, diag))
		...
		df.to_pickle('../Results/emcee_exp_fits/derived_params/subsamp/{0}_{1}.pkl'.format(gal_ID, diag))
